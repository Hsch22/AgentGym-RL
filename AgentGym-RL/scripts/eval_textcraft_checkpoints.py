...
def parse_eval_metrics(log_path: Path):
    ...
    with log_path.open("r", errors="ignore") as f:
        for line in f:
            ...
            m = AVG_RE.match(line)
            if m:
                metric_name = "avg_at_{}".format(m.group(1))
                value = float(m.group(2))
                if category is None:
                    metrics[metric_name] = value
                else:
                    metrics[f"{category}/{metric_name}"] = value
                continue

            m = PASS_RE.match(line)
            if m:
                metric_name = "pass_at_{}".format(m.group(1))
                value = float(m.group(2))
                if category is None:
                    metrics[metric_name] = value
                else:
...
def main():
    ...
    parser.add_argument("--env-addr", default="http://127.0.0.1:36005")
    ...
    results_dir = run_dir / "eval_textcraft_ckpt_sweep"
    ...
    if args.wandb_mode != "disabled":
        ...
        wandb_run = wandb.init(
            ...
            name=args.wandb_name or f"textcraft_eval_{run_dir.name}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}",
            config={
                "run_dir": str(run_dir),
