...
env_server_url="http://127.0.0.1:36025"
...
exp_name="babyai_scalinginter_baseline_3b_$(date +%Y%m%d_%H%M)"
model_save_path="${model_save_dir}/${exp_name}"

mkdir -p "${model_save_path}"

echo "[baseline-run] task=${task_name} model=${pure_agent_model_name}"
echo "[baseline-run] rounds=[6,13,20] train_batch_size=${train_batch_size} rollout_n=${rollout_sample_num}"
echo "[baseline-run] max_prompt_length=512 max_response_length=8192"
